...
# # X_PROBE = 0.22572455617737594 # old [0,1]^2 ∈ R^2 probe location
# X_PROBE = 0.07500000000000001  # new [0,1] x [0,0.2] ∈ R^2 probe location
# F0 = 10.0
...
# User configuration
# ------------------------------------------------------------
VAR = 1  # default signal1 column index in CSV after the time column
# X_PROBE = 0.22572455617737594 # old [0,1]^2 ∈ R^2 probe location
X_PROBE = 0.07500000000000001  # new [0,1] x [0,0.2] ∈ R^2 probe location
F0 = 10.0
...
DEFAULT_T_END = None
DEFAULT_DT_TARGET = None
DEFAULT_REFERENCE_SOURCE = "analytic"
DEFAULT_SIGNAL1_COL = 1
DEFAULT_SIGNAL2_COL = 2
...
def parse_args():
    ...
    parser.add_argument(
        "--reference-source",
        choices=("analytic", "csv"),
        default=DEFAULT_REFERENCE_SOURCE,
        help="How to build signal2.dat: built-in analytical reference or CSV column.",
    )
    parser.add_argument(
        "--signal1-col",
        type=int,
        default=DEFAULT_SIGNAL1_COL,
        help="CSV column index for signal1 (time is always column 0).",
    )
    parser.add_argument(
        "--signal2-col",
        type=int,
        default=DEFAULT_SIGNAL2_COL,
        help="CSV column index for signal2 when --reference-source=csv.",
    )
    ...
    return args, output_file, input_csv
...
def load_csv_signals(
    input_csv: Path,
    signal1_col: int,
    signal2_col: Optional[int] = None,
):
    """
    Load one or two signals from CSV.

    The CSV is assumed to contain:
      column 0          -> time column
      signal1_col       -> signal1 values
      signal2_col       -> signal2 values (optional)
    """
    usecols = [0, signal1_col] if signal2_col is None else [0, signal1_col, signal2_col]

    df = pd.read_csv(input_csv, usecols=usecols, dtype=np.float64, engine="c")

    time = df.iloc[:, 0].to_numpy()
    signal1 = df.iloc[:, 1].to_numpy()
    signal2 = None if signal2_col is None else df.iloc[:, 2].to_numpy()

    if len(time) < 2:
        raise ValueError(f"Not enough rows in file: {input_csv}")

    sort_idx = np.argsort(time)
    time = time[sort_idx]
    signal1 = signal1[sort_idx]
    if signal2 is not None:
        signal2 = signal2[sort_idx]
    ...
    if len(time) < 2:
        raise ValueError(f"Not enough unique time samples in file: {input_csv}")

    return input_csv, time, signal1, signal2
...
# ------------------------------------------------------------
def main():
    args, output_file, input_csv = parse_args()

    local_norm = args.local_norm
    t_start_arg = args.t_start
    t_end_arg = args.t_end
    dt_target_arg = args.dt_target
    reference_source = args.reference_source
    signal1_col = args.signal1_col
    signal2_col = args.signal2_col

    outdir = output_file.parent
    outdir.mkdir(parents=True, exist_ok=True)

    if reference_source == "csv":
        fname, time_num_raw, signal1_raw, signal2_raw = load_csv_signals(
            input_csv,
            signal1_col=signal1_col,
            signal2_col=signal2_col,
        )
    else:
        fname, time_num_raw, signal1_raw, _ = load_csv_signals(
            input_csv,
            signal1_col=signal1_col,
            signal2_col=None,
        )
    ...
    signal1 = interpolate_to_grid(time_num_raw, signal1_raw, tout, kind=INTERP_KIND)

    if reference_source == "csv":
        signal2 = interpolate_to_grid(time_num_raw, signal2_raw, tout, kind=INTERP_KIND)
    else:
        signal2 = analytic_reference_trace(tout)
    ...
    if np.isnan(signal1).any():
        bad = np.count_nonzero(np.isnan(signal1))
        raise ValueError(
            f"Interpolated signal1 contains {bad} NaN values. "
            "Choose a smaller export interval or check the source CSV."
        )

    if np.isnan(signal2).any():
        bad = np.count_nonzero(np.isnan(signal2))
        raise ValueError(
            f"Interpolated signal2 contains {bad} NaN values. "
            "Choose a smaller export interval or check the source CSV."
        )
    ...
    print(f"Initial time      : {t0_out:.16e}")
    print(f"Final time        : {tout[-1]:.16e}")
    print(f"signal1 CSV col   : {signal1_col}")
    if reference_source == "csv":
        print(f"signal2 CSV col   : {signal2_col}")
        print("signal1.dat = interpolated test/numerical signal")
        print("signal2.dat = interpolated CSV reference signal")
    else:
        print("signal1.dat = interpolated numerical signal")
        print("signal2.dat = analytical reference signal")
