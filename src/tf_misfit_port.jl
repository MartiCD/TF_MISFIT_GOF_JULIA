...
#     println("Allocating work arrays"); flush(stdout)
...
#     wv_ref_abs = zeros(Float64, nc, mt, nf_tf)
...
#         #     wv_ref[j, :, :] .= wv2[j, :, :]
#         # end
#         if (!is_s2_reference) && (maximum(abs, @view(wv1[j, :, :])) < maximum(abs, @view(wv2[j, :, :])))
#             @inbounds for i in 1:mt, l in 1:nf_tf
#                 wv_ref_abs[j, i, l] = abs(wv1[j, i, l])
#             end
#         else
#             @inbounds for i in 1:mt, l in 1:nf_tf
#                 wv_ref_abs[j, i, l] = abs(wv2[j, i, l])
#             end
#         end
#     end

#     println("Computing cwt1/cwt2"); flush(stdout)
#     cwt1 = abs.(wv1).^2
#     cwt2 = abs.(wv2).^2

#     maxtf = maximum(wv_ref_abs)
#     # maxtf = maximum(abs.(wv_ref))
#     de .= abs.(wv1) .- abs.(wv2)

#     println("Computing tf misfits"); flush(stdout)
...
#         dp[j, i, l] = wv_ref_abs[j, i, l] * dp[j, i, l]
#     end

#     println("Releasing wv1/wv2"); flush(stdout)
#     wv1 = nothing
#     wv2 = nothing
#     GC.gc()
#     println("Released wv1/wv2"); flush(stdout)
...
#         denoms = sum(@view wv_ref_abs[j, i, :]) / float(nf_tf)
...
#         denoms = sum(@view wv_ref_abs[j, :, l]) / float(mt)
#         maxdenom = max(maxdenom, denoms)
#     end
...
#         denoms = sum(abs2, @view wv_ref_abs[j, :, :])
#         maxdenom = max(maxdenom, denoms)
#     end
